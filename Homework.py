...
#         self.celsius = c
#
# class kelvin(convert):
#     def kelvingrad(self):
#         return self.celsius+273.15
#
# class fehrenheit(convert):
#     def fahrenheitgrad(self):
#         return self.celsius
#
# def test_hw3():
#     t1 = kelvin(10)
...
class Bots:
    ...
    def __init__(self, reyt, stil, pizzerias):
        self.__reyt = reyt
        self.__stil = stil
        for pizzeria in pizzerias:
            if stil > pizzeria.stills:
                continue
            if reyt<=pizzeria.reyting:
                print(f"{stil} tables in the {pizzeria.name} pizzeria with rate {pizzeria.reyting} has reserved")
                break
        else:
            print(f"There is {pizzeria.name} pizzeria that matching with your {reyt} rate, but there is only {pizzeria.stills} free tables")


def test_task2():
    a= Zvd('PizzaHut', 4.0, 10)
    b= Zvd('Chelentano', 4.0, 11)
    c= Zvd('Il Molino', 4.9, 12)
    z = Bots(4.0, 13, [a, b, c])
